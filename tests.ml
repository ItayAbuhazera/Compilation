...
(* A test function that takes a quadruple and tests each stage of the pipeline *)
open Tag_Parser;;
open Reader;;
open PC;;
open Semantic_Analysis;;

(* Enhanced test pipeline function with exception handling for more intuitive error messages. *)
let test_pipeline n (input_to_reader, expected_reader_output, expected_parser_output, expected_lexical_output) =
  try
    let reader_output = read input_to_reader in
    if reader_output <> expected_reader_output then
      begin
        Printf.printf "Test #%d - Reader output mismatch.\nExpected: %s\nGot: %s\n" n (string_of_sexpr expected_reader_output) (string_of_sexpr reader_output);
        false
      end
    else
      let parser_output = tag_parse reader_output in
      if parser_output <> expected_parser_output then
        begin
          Printf.printf "Test #%d - Parser output mismatch.\nExpected: %s\nGot: %s\n" n (string_of_expr expected_parser_output) (string_of_expr parser_output);
          false
        end
      else
        let lexical_output = annotate_lexical_address parser_output in
        if lexical_output <> expected_lexical_output then
          begin
            Printf.printf "Test #%d - Lexical annotate output mismatch.\nExpected: %s\nGot: %s\n" n (string_of_expr' expected_lexical_output) (string_of_expr' lexical_output);
            false
          end
        else
          true
  with
  | X_not_yet_implemented msg -> Printf.printf "Test #%d - Exception: %s\n" n msg; false
  | e -> Printf.printf "Test #%d - Unexpected exception: %s\n" n (Printexc.to_string e); false


(* Runs all tests with test numbering and reports the number of passed and failed tests. *)
let test_all () =
  let results = List.mapi test_pipeline quadruples in
  let passed, failed = List.fold_left (fun (pass, fail) result -> if result then (pass + 1, fail) else (pass, fail + 1)) (0, 0) results in
  if failed = 0 then
    Printf.printf "All tests passed. Total: %d\n" (passed + failed)
  else
    Printf.printf "%d/%d tests passed. %d tests failed.\n" passed (passed + failed) failed;;
test_all ();;
